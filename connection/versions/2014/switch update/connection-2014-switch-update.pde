  ...
  if (connectobjs.get(si).isfunc) {
    if (!connectobjs.get(si).execed) {
      ...
      if (
          ...
          connectobjs.get(si).val.equals("case") ||
          connectobjs.get(si).val.equals("endswitch")) {
      ...
      } else if (connectobjs.get(si).val.equals("switch")) { // switch! 'twas very hard to implement :P
        if (connectobjs.get(si).intocons.size() > 0) { // if there was a parameter connected
          for (int i = 0; i < connectobjs.get(si).dcons.size(); i++) { // check case
            if (
                connectobjs.get(connectobjs.get(si).dcons.get(i)).val.equals("case") && // is the dcon object a case object?
                connectobjs.get(connectobjs.get(si).dcons.get(i)).intocons.size() > 0 // is the object connect to parameters?
                ) {
              if (varobjs.get(connectobjs.get(si).intocons.get(0)).val.equals(varobjs.get(connectobjs.get(connectobjs.get(si).dcons.get(i)).intocons.get(0)).val)) { // is the switch-connected parameter equal to the case-connected parameter?
                if (!connectobjs.get(si).execmsgs.hasValue(connectobjs.get(si).dcons.get(i))) { // add to execmsg!
                  connectobjs.get(si).execmsgs.append(connectobjs.get(si).dcons.get(i));
                  break;
                }
              }
          ...
          for (int i = 0; i < connectobjs.get(si).dcons.size(); i++) { // process endswitch
            if (!connectobjs.get(si).execmsgs.hasValue(connectobjs.get(si).dcons.get(i)) && connectobjs.get(connectobjs.get(si).dcons.get(i)).val.equals("endswitch")) {
              connectobjs.get(si).execmsgs.append(connectobjs.get(si).dcons.get(i));
              break;
            }
          }
